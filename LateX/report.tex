...
\begin{document}
    ...
    Denote each of the input sample as $\mathbf{x_i}$ and each of the principle components as $\mathbf{p_j}$, where $\mathbf{x_i}, \mathbf{p_j} \in \mathbf{F}^{m}$. If we constrain our choice of principle components to orthonormal vectors, we can write each of input sample, $\mathbf{x_i}$ as follows:
    %
    \begin{equation}
    ...
    \end{equation}
    	...
    	Each $\mathbf{y_i}$ vector can be computed with a matrix equation. Let $\mathbf{X} \in \mathbf{F}^{m \times n}$ be the matrix formed by concatenating each sample $\mathbf{x_i}$.
    %
    \begin{equation}
    	...
    	\end{equation}
    	%
    	Similary, let $\mathbf{P} \in \mathbf{F}^{m \times m}$ be the matrix formed by concatentating the principle components $\mathbf{p_j}$ and $\mathbf{Y} \in \mathbf{F}^{m \times n}$ be the matrix formed by concatenating the resulting weight vectors $\mathbf{y_i}$. Then, the following matrix equation should compute each $\mathbf{y_i}$ vector:
    	%
    	\begin{equation}
    		\mathbf{Y} = \mathbf{P^{H}}\mathbf{X}
    	\end{equation}
